...
SELECT m.name, m.brand_name, b.headquarters FROM brands as b JOIN models as m ON (b.name=m.brand_name) WHERE m.name = 'Mustang' and m.brand_name = 'Ford';
...
ON m.brand_name=b.name WHERE m.year IN (SELECT founded FROM brands);